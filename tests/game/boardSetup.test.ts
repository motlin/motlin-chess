...
describe('createInitialBoard', () => {
	...
	test('8x8 board has correct back rank pieces', () => {
		...
		expect(board.get(0, 0)).toStrictEqual({type: 'rook', color: 'black', hasMoved: false});
		expect(board.get(0, 1)).toStrictEqual({type: 'knight', color: 'black', hasMoved: false});
		expect(board.get(0, 2)).toStrictEqual({type: 'bishop', color: 'black', hasMoved: false});
		expect(board.get(0, 3)).toStrictEqual({type: 'queen', color: 'black', hasMoved: false});
		expect(board.get(0, 4)).toStrictEqual({type: 'king', color: 'black', hasMoved: false});
		expect(board.get(0, 5)).toStrictEqual({type: 'bishop', color: 'black', hasMoved: false});
		expect(board.get(0, 6)).toStrictEqual({type: 'knight', color: 'black', hasMoved: false});
		expect(board.get(0, 7)).toStrictEqual({type: 'rook', color: 'black', hasMoved: false});

		expect(board.get(7, 0)).toStrictEqual({type: 'rook', color: 'white', hasMoved: false});
		expect(board.get(7, 4)).toStrictEqual({type: 'king', color: 'white', hasMoved: false});
	});

	test('8x8 board has pawns on rows 1 and 6', () => {
		const board = createInitialBoard(8, STD);
		for (let col = 0; col < 8; col++) {
			expect(board.get(1, col)).toStrictEqual({type: 'pawn', color: 'black', hasMoved: false});
			expect(board.get(6, col)).toStrictEqual({type: 'pawn', color: 'white', hasMoved: false});
		}
	});
	...
	test('10x10 board centers standard pieces', () => {
		...
		expect(board.get(0, 1)).toStrictEqual({type: 'rook', color: 'black', hasMoved: false});
		expect(board.get(0, 5)).toStrictEqual({type: 'king', color: 'black', hasMoved: false});
		expect(board.get(0, 8)).toStrictEqual({type: 'rook', color: 'black', hasMoved: false});
		expect(board.get(0, 9)).toBeNull();

		for (let col = 0; col < 10; col++) {
			expect(board.get(1, col)).toStrictEqual({type: 'pawn', color: 'black', hasMoved: false});
			expect(board.get(8, col)).toStrictEqual({type: 'pawn', color: 'white', hasMoved: false});
		}
	});
	...
	test('12x12 board has correct layout', () => {
		...
		expect(board.get(0, 2)).toStrictEqual({type: 'rook', color: 'black', hasMoved: false});
		expect(board.get(0, 6)).toStrictEqual({type: 'king', color: 'black', hasMoved: false});
		expect(board.get(0, 9)).toStrictEqual({type: 'rook', color: 'black', hasMoved: false});
		expect(board.get(0, 10)).toBeNull();
		expect(board.get(0, 11)).toBeNull();
	});
	...
	test('4x4 board with R B K R layout', () => {
		...
		expect(board.get(0, 0)?.type).toBe('rook');
		expect(board.get(0, 1)?.type).toBe('bishop');
		expect(board.get(0, 2)?.type).toBe('king');
		expect(board.get(0, 3)?.type).toBe('rook');

		for (let col = 0; col < 4; col++) {
			expect(board.get(1, col)?.type).toBe('pawn');
			expect(board.get(2, col)?.type).toBe('pawn');
		}
	});
	...
	test('6x6 board with R B Q K B R layout', () => {
		...
		expect(board.get(0, 0)?.type).toBe('rook');
		expect(board.get(0, 1)?.type).toBe('bishop');
		expect(board.get(0, 2)?.type).toBe('queen');
		expect(board.get(0, 3)?.type).toBe('king');
		expect(board.get(0, 4)?.type).toBe('bishop');
		expect(board.get(0, 5)?.type).toBe('rook');
	});
});
